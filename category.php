...
<body>
    <div class="container">
        <h2>List Per Category</h2>
    </div>
    <div class="category">
        <?php
            $sql = "SELECT * FROM inventory";
            $lines = $connect->query($sql);
            
                while($row = $lines->fetch_object()){
        ?>
            <a href="category.php?cate=<?=$row->category?>"><?=$row->category?></a>
        <?php
            }
            if(isset($_GET['cate'])){
                $categ = $_GET['cate'];
                $sql = "SELECT * FROM inventory WHERE category = '$categ'";
            }else{
                $sql = "SELECT * FROM inventory";
            }
